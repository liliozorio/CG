...
// CREATE PLANE
function createGroundPlaneXZ(position, width=SIZE_PLANE + 1, height=SIZE_PLANE + 1, widthSegments = 10, heightSegments = 10, gcolor = null) {
    ...
    console
    plane.matrix.multiply(mat4.makeTranslation(position.x, position.y, position.z));
...
function createChambers() {
    const planePositions = {
        ...
        p2: { x: 0, y: -3, z: -SIZE_PLANE-6.5 },
        p3: { x: 0.0, y: 3.1, z: SIZE_PLANE+6.7 },
        p4: { x: SIZE_PLANE+6.4, y: -3, z: 0.0 },
        p5: { x: -SIZE_PLANE-6.4, y: 3.1, z: 0.0 },
    };
    for (let i = 1; i < 6; i++) {
        let plane = createGroundPlaneXZ(planePositions["p"+i]);
        scene.add(plane);
    }
    //console.log(plane.position)

    makeFloor({ x: 0, y: 0.05, z: 0 });
...
function insertStairs() {
    ...
    escadaFinal.position.set(-23.5, 0, 0);
